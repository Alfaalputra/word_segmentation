...
class Correction:
    ...
    def __init__(self, corpus="english"):
        ...
        """
        super().__init__()
        self.WORDS = Counter(helpers.read_stats(corpus, 1))
        self.N = sum(self.WORDS.values())

    @staticmethod
    def tokens(text):
        return REGEX_TOKEN.findall(text)
    ...
    def correct_match(self, match):
        ...
        word = match.group()
        return self.case_of(word)(self.correct(word))
    ...
    def correct_word(self, word, assume_wrong=False, fast=False):
        ...
        return self.case_of(word)(self.correct(word, assume_wrong=assume_wrong, fast=fast))
    ...
    def normalize_elongated(self, word):
        return self.case_of(word)(self.best_elong_candidate(word))
